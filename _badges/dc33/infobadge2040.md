...
research:
  ...
  notes: >-
    Core specs (RP2040, 2.9" 4-color e-ink, UART programming, $50) confirmed by both the
    Tindie storefront and the maker's own site (thebadg.es). No GitHub repo, Hackerday.io
    project, or published hardware/firmware files were found, so make_your_own fields are
    left empty rather than guessed. Battery/power detail is the maker's own phrasing
    ("battery-less design with low-power e-ink display") and is a little ambiguous about
    how the badge is actually powered day-to-day; left as-is rather than interpreted.
    No LED info found. Status set to released since this was sold and delivered at DEF
    CON 33 (not merely announced), and get_one.availability set to sold_out per the Tindie
