---
title: Sneaky Badge
id: dc31-sneakers-badge
layout: badge
parent: DC31
grand_parent: Badge Archive
nav_exclude: true
type: badge
event: dc31
year: 2023
makers:
- name: Gigs
summary: A Raspberry Pi Pico badge paying homage to the 1992 hacker film "Sneakers," with an OLED screen and plug-in scrabble-style letter tiles for word puzzles.
functions: Word-game puzzle challenges built into the firmware; 14 tile slots on the badge take custom scrabble-style letters (56 tiles included) to spell answers.
look:
  colors: []
  shape: null
  themes:
  - movie
  - puzzle
tech:
  mcu: Raspberry Pi Pico
  leds: null
  display: OLED
  connectivity: []
  battery: null
  sao_version: null
get_one:
  price: $120.00 (Hacker Warehouse listing; community sheet quoted $140.00)
  price_usd: 120.0
  quantity: ''
  availability: sold_out
  availability_note: Hacker Warehouse product page showed "out of stock" as of 2026-09-06.
  distribution:
  - purchase
  where: Sold through Hacker Warehouse's online store and in person at their DEF CON 31 booth.
make_your_own:
  open_source: null
  hardware_url: null
  firmware_url: null
  eda_tool: null
links:
- kind: store
  url: https://hackerwarehouse.com/product/sneaky-badge/
  label: Sneaky Badge - Hacker Warehouse
images:
- file: assets/images/badges/dc31/sneakers-badge/14fb38c1da.jpg
  source: "https://hackerwarehouse.com/product/sneaky-badge/"
  credit: "Hacker Warehouse"
  caption: "Sneaky Badge, a Sneakers (1992) film homage badge sold by Hacker Warehouse"
- file: assets/images/badges/dc31/sneakers-badge/3ef2aaf51b.jpg
  source: "https://hackerwarehouse.com/product/sneaky-badge/"
  credit: "Hacker Warehouse"
  caption: "Sneaky Badge showing the OLED screen and scrabble-style letter tile slots"
contact: {}
notes:
- This badge pays homage to the hacker favorite movie "Sneakers".
- The community sheet titled this "Sneakers Badge"; Hacker Warehouse sells it as the "Sneaky Badge" (SKU GS-SB).
status: released
sources:
- kind: sheet
  event: dc31
  row: 44
  updated: '2023-07-25'
- kind: url
  url: https://hackerwarehouse.com/product/sneaky-badge/
  title: Sneaky Badge - Hacker Warehouse
  accessed: '2026-09-06'
  note: Confirms product name, description (Sneakers film homage, Raspberry Pi Pico, OLED, scrabble-style tiles, 14 slots/56 tiles), price $120, SKU GS-SB, out-of-stock status, and product photos.
research:
  status: researched
  confidence: medium
  last_checked: '2026-09-06'
  notes: >-
    No maker page or press coverage found; identification rests on the Hacker Warehouse
    storefront listing plus its SKU prefix "GS-SB". Other Hacker Warehouse badge SKUs
    follow a maker-initials pattern (e.g. "SS-TB" for the Tipsy Badge, credited on this
    archive to maker "seeess"), and "GS" matches "Gigs," the maker the community sheet
    named for this row, which supports the identification without being maker-confirmed.
    The listing's product photos carry a September 2025 upload date, which may just be
    when Hacker Warehouse re-photographed/re-listed an older item rather than evidence
    of a different release year; the sheet's DC31 (2023) placement and $140 quoted price
    vs. the store's $120 could not be reconciled from available sources, so both are
    recorded. Quantity made, LED count, connectivity, and power/battery details were not
    stated anywhere found. No hardware/firmware files were found, so make_your_own is
    left empty rather than guessed.
last_modified_date: '2026-09-06'
---

The Sneaky Badge is Hacker Warehouse's tribute to the 1992 hacker heist film
"Sneakers," built around a Raspberry Pi Pico with an OLED screen. Rather than
just displaying a static homage, the badge is a puzzle device: it has 14 slots
that hold custom, plug-in scrabble-style letter tiles (56 tiles ship with the
badge), and its firmware runs word-game puzzle challenges that use those
letters. It was sold through Hacker Warehouse's online store and at their
booth at DEF CON 31 for $120, alongside their other badgelife offerings that
year.

The community badge sheet that seeded this entry listed the item as "Sneakers
Badge" by maker "Gigs" at a quoted $140, which differs slightly from the
storefront's name and price. The Hacker Warehouse SKU for the product, "GS-SB,"
lines up with a maker-initials pattern this archive has seen elsewhere in
their catalog (their Tipsy Badge, made by "seeess," carries SKU "SS-TB"),
which is consistent with "Gigs" as the designer even though the product page
itself does not credit a maker by name.

As of the most recent check, the badge is listed as out of stock. No separate
maker page, GitHub repo, or press coverage was found, so details like LED
count, connectivity, and battery/power are not recorded here.
