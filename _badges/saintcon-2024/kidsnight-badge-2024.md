...
research:
  ...
  notes: >-
    Confirmed via the maker's own repo (utahsaint-org/MiniBadges2024) that this is a real minibadge
    made for the SAINTCON 2024 Kids Night event, part of the community's official 2024 minibadge
    collection. The KiCad schematic shows an ATtiny85 MCU, three discrete (non-addressable) LEDs,
    a battery, and a resistor, driven off a SAINTCON "MiniBadge" connector part (not a standard SAO
    header) -- consistent with SAINTCON's own klip-on minibadge system rather than the badge.life SAO
    standard. No firmware source was found in the repo (hardware/PCB files only: KiCad project,
    Gerbers, and vector art), so open_source is marked partial rather than yes. Could not confirm: an
    individual designer/maker name (the repo is published under the community org account, not a
    named person), colors/shape (no raster photos exist in the repo -- only SVG and Adobe Illustrator
    vector files, which the archive's image tool cannot save as photos), price, quantity made, or how
    it was distributed at the event (SAINTCON minibadges are generally free/traded/earned per
    saintcon.org, but nothing ties that specifically to this badge). A 38MB official "2024 SAINTCON
    MiniBadge Guide" PDF in a companion repo likely documents this badge in more depth but was too
    large to fetch within budget.
