...
research:
  ...
  notes: 'Fact-check pass (2026-09-08): confirmed directly against grandideastudio.com/portfolio/other/defcon-27-badge/ (maker''s own page, fetched in full) — MCU, LED driver, battery, gemstone, the ten attendee-type color variants including "Uber," and 28,600 total units all match verbatim. The saved group-photo image was downloaded and pixel-matched against the same page''s third photo, confirming it genuinely shows the black PCB variant (row 2, right) as captioned. forum.defcon.org/node/229722 was fetched in full (an Ars Technica repost) and confirms Joe Grand ("Kingpin") as designer, returning after DC14-18 at Jeff Moss''s request; note that article''s own use of "uber badge" refers informally to attendees'' self-hacked badges that cycle through identities, an unrelated coincidental usage, not the official color tier, so it neither confirms nor contradicts the Uber-variant claim (which rests on the maker''s page instead). Both Worthpoint URLs remain blocked by PerimeterX captcha on refetch (403/human-verification wall); the ~20-units and "unpopulated PCB" claims are still search-snippet-sourced only, as already disclosed, and confidence is kept at medium for that reason. The duplicate flag against dc27-badge-2019 was checked against that entry directly and is correct (same maker, id, and tech). One minor gap noted but not changed: the body text mentions the maker''s NXP NXH2261 NFMI transceiver, but tech.connectivity is "none" because the guide''s connectivity vocabulary has no NFMI/proximity-badge term to select instead — left as-is rather than inventing one.'
