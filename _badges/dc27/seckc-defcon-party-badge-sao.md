...
- file: assets/images/badges/dc27/seckc-defcon-party-badge-sao/a9dbac7151.jpg
  source: https://www.tindie.com/products/badgepirates/defcon-27-seckc-party-star/
  credit: BadgePirates
...
- 'This is a duplicate of an already-researched entry: dc27-seckc-defcon-party-badge-sao ("DefCon 27 SecKC Party Star"), which points at the same GitHub repo (BadgePiratesLLC/DefCon_SecKCPartyStar_27) and the same Tindie listing. That entry''s own notes record it as itself a merge of two prior duplicate stubs of this same item. Core facts here (maker, event, look, tech, get_one, links) were copied from that verified entry rather than re-researched from scratch, per the research guide''s instruction to fill in duplicates and report them.'
- The sweep imported this stub under the GitHub repo's raw slug "DefCon_SecKCPartyStar_27"; the maker's own naming on Tindie and in the repo description is "DefCon 27 SecKC Party Star", used here as the title.
status: released
sources:
- kind: url
  url: https://store.badgepirates.com/product/preorder-seckc-defcon-party-badge-sao/
...
research:
  ...
  notes: 'Fact-check pass (2026-09-08): confirmed Tindie listing (product name, $4.99 price, broken/bare-board resale stock, star shape with blinking LEDs), GitHub repo (archived, "Party Star ''badge''" per README), and docs.badgepirates.com catalog ("Party Star variant" of the DC27 conference badge). Corrected a factual error: the prior draft claimed the GitHub repo held only KiCad footprints with no schematic or BOM; the repo actually contains a full schematic, PCB layout, gerbers, and a BOM (ibom.html) -- only firmware is genuinely absent, consistent with the board carrying no MCU. store.badgepirates.com still does not resolve (confirmed again). Original production quantity, price, and exact distribution method at the 2019 SecKC party remain undocumented; LED count/driver type undocumented (left blank rather than read off raw schematic files, which would be new research beyond this pass). Likely a duplicate of the dc27-seckc-party-star stub entry, which points at the same Tindie listing
    -- flagged for a human to dedupe, not resolved here. Merged with duplicate entry ''DefCon 27 SecKC Party Star'' (dc27-seckc-party-star). Merged with duplicate entry ''DefCon 27 SecKC Party Star'' (dc27-defcon-seckcpartystar-27).'
...
## Notes merged from the duplicate entry "DefCon 27 SecKC Party Star"
...
BadgePirates made this sheriff-star-shaped party badge for the SecKC crew's gathering at DEF CON 27 in 2019. The design is a gold-plated star on black PCB with lights built into the star's points. The GitHub repo's promotional artwork is filed as an "SAO" banner, but the maker's storefront and repo describe it as a standalone party badge/keepsake rather than a plug-in SAO.

The hardware files (KiCad schematic, PCB layout, footprints, and documents, but no firmware) are published on GitHub; the repo is archived and gives no further build narrative. In 2021, BadgePirates resold a small remaining stock of these badges on Tindie at $4.99 each — some non-working assembled units, some bare unpopulated boards — as leftovers found in storage, rather than as new production.

This entry duplicates dc27-seckc-defcon-party-badge-sao, an already-researched entry for the same item; see that entry for the fuller writeup and fact-check history.
