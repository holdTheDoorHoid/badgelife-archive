...
tech:
  ...
  leds:
    count: null
    type: RGB
    note: Addressable RGB LEDs run along the wrench's edge via Espressif's led_strip
      driver; used for faction colors, a boot-progress gradient, and game-event flashes.
      Exact LED count not stated in the sources.
