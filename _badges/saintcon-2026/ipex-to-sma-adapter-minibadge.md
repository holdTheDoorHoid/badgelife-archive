...
id: saintcon-2026-ipex-to-sma-adapter-minibadge
layout: badge
...
research:
  ...
  notes: 'Fact-check pass (2026-09-07): confirmed the search page at minibadge.wiki/?search=... renders client-side and returns "No minibadges match your filters yet." in its static HTML, matching the prior note. Confirmed every populated field (title, maker, description, soldering instructions, quantity 20, board house JLCPCB, conference year 2026, howToAcquire text) against the raw record for this item in minibadge.wiki/2026.json. Verified both saved images (front/back) are the same renders served at minibadge.wiki/images/2026/ipex-to-sma-adapter-minibadge-front.png and -back.png. Removed one unsupported editorial phrase from the body ("instead of a fragile flying lead") that was not stated by any source. No independent Tindie listing, price, or availability status could be confirmed, so those fields remain empty/unknown; confidence stays medium because the only source is the maker''s own wiki submission, not an independent storefront or press confirmation. The maker (Pips) submitted several other SAINTCON 2026 minibadges to the same wiki, including an "ESP32 Meshtastic Minibadge" that this adapter is explicitly designed to pair with.'
last_modified_date: '2026-09-07'
---

The IPEX to SMA adapter Minibadge is a small passive accessory board made by Pips for SAINTCON 2026. It solves one specific problem: it converts an I-PEX (U.FL) antenna connector to a standard SMA connector, so a badge or minibadge built around a U.FL-only radio module can use a normal screw-on vertical SMA antenna. The maker built it specifically as a companion to their own ESP32 Meshtastic Minibadge from the same year.
