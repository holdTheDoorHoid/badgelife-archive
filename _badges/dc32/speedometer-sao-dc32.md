...
get_one:
  ...
  distribution:
  - village
  where: Available for purchase at the Car Hacking Village (CHV) at DEF CON 32
make_your_own:
  open_source: partial
  hardware_url: null
...
- Maker's name "Uberwoozle" could not be found on Tindie, GitHub, or Hackaday.io as of the last check; no independent storefront or project page was located.
status: listed
sources:
- kind: url
  url: https://github.com/car-hacking-village/DC32_CHV_Speedometer_Firmware
...
- kind: url
  ...
  accessed: '2026-09-07'
  note: Checked for mention of this SAO; the current site covers the 2023 badge and the SAO spec update, but has no listing or archive page for a DC32 (2024) speedometer SAO or its maker.
research:
  status: verified
  confidence: medium
  last_checked: '2026-09-07'
  notes: 'Fact-check pass (2026-09-07): re-fetched all four cited sources plus the raw firmware repo tree/files on GitHub. Confirmed directly: ESP32 target (sdkconfig.defaults'' "ESP32-specific" section, main.c''s ESP-IDF/FreeRTOS includes), the st7565.c/catface_helper.c/speedometer_helper.c/ can_helper.c/cat_battle_msgs.c file set, CAN bus via the ESP32 TWAI driver, a rock-paper-scissors cat-battle exchange (cat_battle_msgs.c''s get_rps/didWinRPS), a speedometer needle-angle lookup table (speedometer_helper.c), the MIT license, the CTF challenge "RPS FTW (Speedometer SAO)" submitted by Uberwoozle with a flag hidden in the on-board font library, and the org''s DC31/32/33 badge lineage. One error was found and corrected: the CHV SAO spec does NOT repurpose the SAO''s I2C pins for CAN. Per the spec''s own pin table, pins 3-4 (the normal I2C pins) are left not-connected, and CAN TX/RX instead run on pins 5-6 (the standard''s two general-purpose GPIOs). Sources and body text were corrected accordingly.
    No hardware repo, price, quantity, LED count, board color/shape, or photo of the physical SAO was found; only firmware-embedded animation frames (image/*.bmp in the repo) were located, and those are display assets, not photos of the item, so no image was saved. `tech.connectivity` is left empty because CAN bus is not in the guide''s allowed connectivity vocabulary. This entry is very likely a duplicate of dc32-speedometer-sao-not-a-cat (same maker, event, LCD display, and cat-battle function); left as a flagged duplicate per the researcher''s note and the guide''s "do not touch other entries" rule — not merged or deleted here. Merged with duplicate entry ''Speedometer SAO (not a cat)'' (dc32-speedometer-sao-not-a-cat).'
last_modified_date: '2026-09-07'
redirect_from:
- /badges/dc32/speedometer-sao-not-a-cat/
---
...
## Notes merged from the duplicate entry "Speedometer SAO (not a cat)"

This SAO was sold at the Car Hacking Village (CHV) at DEF CON 32 (2024) by a maker credited on the community badge sheet as "Uberwoozle." Per the sheet, it has an LCD display, functions as an official CHV speedometer, and includes a "cat battles" feature — playful details that, combined with the title's parenthetical "(not a cat)," hint at some joke about the badge being mistaken for a cat-shaped piece, though no source was found to confirm what that joke actually is. It sold for $50.

Beyond the original sheet entry, no maker storefront, project page, or press coverage could be located for this item or for "Uberwoozle" as a maker name — searches of Tindie, GitHub, and Hackaday.io turned up nothing, and the Car Hacking Village's current website does not have an archived page for DEF CON 32 badges. Technical details such as the microcontroller, LED count, quantity made, and current availability remain unknown.
