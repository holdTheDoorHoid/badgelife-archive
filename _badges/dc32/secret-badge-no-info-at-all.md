...
research:
  ...
  notes: >-
    This sheet row is a duplicate of dc32-another-badge-similar-to-past-years: both are
    "Darknet (Gulo)" rows for dc32 (sheet rows 42 and 43) pointing at the same
    darknet-ng-13-badge blog post, which is categorized "DEF CON 32" and confirms this is
    the 2024 badge. The sheet appears to have logged the pre-announcement teaser (this row,
    originally titled "Secret Badge (no info at all)", with contest/XP teaser text) and the
    finished badge announcement as two separate rows. Retitled from the sheet's placeholder
    wording to the maker's actual project name and filled in identically to the other entry.
    No exact price, total quantity, or badge-specific firmware repo was found (Meshtastic is
    third-party, not a Darknet-NG fork).
