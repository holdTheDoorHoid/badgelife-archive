...
makers:
- name: TwinkleTwinkie
summary: A Simple Add-On badge shaped like a cartoonish take on "The Child" (Grogu) from The Mandalorian, with four randomly flashing RGB LEDs.
...
tech:
  ...
  leds:
    count: 4
    type: RGB
    note: Fast random flashing pattern; no microcontroller mentioned by the maker.
  display: null
  connectivity: []
  battery: null
  sao_version: v2
...
research:
  ...
  notes: The Tindie listing is the only source found; it does not name a specific convention or year the badge was designed for, only a June 2021 listing/photo date, so year is set from that and event is left as "other". No MCU, design files, price, or production quantity are disclosed by the maker. A WebSearch budget limit prevented additional searches beyond the product page fetch.
