...
research:
  ...
  notes: >-
    The repo (firmware + KiCad PCB source) is the only real documentation found; there is no README,
    project write-up, price, quantity, or photo of the finished board. The public AramCon add-on
    catalog page does not list this keyboard add-on among the badge team's official SAOs, so it may
    have been a smaller/limited build rather than a mass-distributed give-away. No AramCon event id
    exists yet in this archive's events.yml, so event is left as "other"; best evidence points to the
    "AramCon 2" (2020) badge generation. Could not confirm price, quantity made, or availability.
