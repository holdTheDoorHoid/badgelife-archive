...
id: other-winter-hacker-fest-pin
layout: badge
parent: Other
...
The Winter Hacker Fest Pin is listed on the community badge sheet as a collaboration between KBEmbedded and a co-maker known as "hunny," with kbembedded.com given as the only link. KBEmbedded is the handle of Kris Bahnsen, a hardware/firmware developer best known for Flipper Zero accessories (Game Boy trading and printer tools, an RFID cloner, and the FLICK frequency-country kit); none of those public projects reference a pin, badge, or SAO, and the kbembedded.com site itself currently has no blog posts or product pages describing this item.

Nothing about the pin's design, features, electronics, event, or availability could be confirmed from sources actually read during this pass, so those fields are left empty rather than guessed. The entry needs a follow-up research pass with working web search (this session's search budget was exhausted and direct search-engine fetches were blocked by CAPTCHAs) to locate the original announcement — likely a social media post or a since-removed page — that the community sheet drew from.
